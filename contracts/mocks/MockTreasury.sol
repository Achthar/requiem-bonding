...
contract MockOlympusTreasury is Ownable {
  ...
  address public sREQ;
  uint256 public sREQQueue; // Delays change to sREQ address
  ...
  constructor(
    ...
    // isLiquidityToken[ _REQDAI ] = true;
    // liquidityTokens.push( _REQDAI );
  ...
  function deposit(
    ...
    IERC20Mintable(REQ).mint(msg.sender, send_);
  ...
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IREQERC20(REQ).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sREQ).balanceOf(msg.sender); // Can only borrow against sREQ held
  ...
  function repayDebtWithREQ(uint256 _amount) external {
    ...
    IREQERC20(REQ).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, REQ, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(REQ).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(REQ).totalSupply().sub(totalDebt));
  }
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match REQ decimals
      value_ = _amount.mul(10**IERC20(REQ).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SREQ) {
      // 9
      sREQQueue = block.number.add(blocksNeededForQueue);
    } else return false;
